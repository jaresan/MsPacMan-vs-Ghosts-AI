...
public abstract class UninformedGraphSearch<PATH_FINDER_CONFIG> extends UninformedGraphSearchBase<PATH_FINDER_CONFIG> {
	...
	protected Collection<SearchTreeNode> getNewOpened(SearchTreeNode toExpand) {
	...
	protected Path getFoundPath(SearchTreeNode target) {
		ArrayList<Node> path = new ArrayList<>();

		path.add(target.node);
		SearchTreeNode current = target;
		while (this.start != current.node) {
